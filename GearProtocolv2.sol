...
/*
 * An Autonomous DeFi Governance System
 * 
 *  Official Website: 
...
contract GearProtocolv2 is ERC20Detailed 
{
    ...
    string constant tokenName = "GearProtocol v2";
    ...
    constructor() public payable ERC20Detailed(tokenName, tokenSymbol, tokenDecimals) 
    {
        ...
        _supply(msg.sender, 1000000*(10**uint256(tokenDecimals)));
    }
    ...
    /*
    *   transfer incurring a feee of 3%
    *   the receiver gets 97% of the sent value
    *   3% is distributed to GEAR farming pool
    */
    function _executeTransfer(address from, address to, uint256 value) private
    {
        ...
        if(fus_total > 0)
        {
            uint256 farmingRewards = threePercent;
    ...
    function harvest() public
    {
        ...
        onePercent = rewards.mul(1).div(100);
        
       //set a minimum  rate to prevent no harvest-fee-txs due to precision loss
            if(onePercent == 0 && rewards > 0) {
                onePercent = 1;
        }
        
        reward = rewards.sub(onePercent);
        ...
        balances[contractOwner] = balances[contractOwner].add(onePercent);
        emit Transfer(address(this), msg.sender, rewards);
        emit Transfer(address(this), contractOwner, onePercent);
    }
